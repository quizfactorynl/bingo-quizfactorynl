...
const loginUser = async (uid: string, password: string) => {
...
export default function Login() {
  ...
  useEffect(
    () =>
      setIsValidUser(email == (process.env.NEXT_PUBLIC_ADMIN_EMAIL as string)),
    [email],
  );
    ...
    >
      ...
      >
        ...
        <Input
          ...
          type="password"
        />
        {isValidUser && <Link color={"blue.50"}>Forgot password?</Link>}
        <Button
          colorScheme="yellow"
          color={"white"}
          ...
          onClick={() => {
            if (email != (process.env.NEXT_PUBLIC_ADMIN_EMAIL as string))
        ...
        {loginState.message.length > 0 && (
          <Text color={"red.300"}>{loginState.message}</Text>
        )}
      </Flex>
    </Flex>
  );
