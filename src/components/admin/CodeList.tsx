import { Box, Flex, IconButton, useDisclosure, useMediaQuery, useToast } from "@chakra-ui/react";
  ...
  return (
    <Flex p={isUnder600 ? 2 : 10} width={"100%"} height={"100%"}>
      ...
      >
        ...
        <Flex width={"100%"} bg={"white"} rounded={"lg"} p={2} flexWrap="wrap"
            shadow={"dark-lg"}
        >
          {codes.map((code, idx) => {
    ...
    return (
      <>
        ...
        <Box
          ...
          flex={1}
          width={{ base: "100%", sm: "calc(50% - 8px)", md: "calc(33.333% - 12px)" }}
          p={4}
          boxShadow={"md"}
          borderRadius={"md"}
          my={2}
