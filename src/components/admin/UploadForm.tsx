    ...
    <form
      ...
      onSubmit={(e) => {
        ...
        const uploadPromises = musics.map((m) => {
          return axios
            .post(`${API_ROUTES.MUSICS}/${musicContext.id}`, {
              title: m.title,
              artist: m.artist,
            ...
            .then((res) => {
              ...
              if (res.data.acknowledged) {
                ...
                setSuccessLogs((l) => {
                  l.push(
                    `At Entry title: ${m.title}, artist: ${m.artist} successfully uploaded`,
                  );
                  return l;
                });
              }
            ...
            .catch((err) => {
              ...
              setLogs((l) => {
                l.push(
                  `At Entry title: ${m.title}, artist: ${m.artist} an error '${err.response.data.error}' occur can't upload it `,
                );
                return l;
              });
            });
    ...
    >
      <FormControl isInvalid={isInvalidFile}>
        ...
        >
          <Image
            width="80"
            height="80"
            src="https://img.icons8.com/officel/80/export-csv.png"
            alt="export-csv"
          />
          {musics.length > 0 ? (
            <>
        ...
        <FormHelperText>
          <Stack gap={"0.4rem"} mt={"1rem"}>
            ...
            <Text>
              <strong>musicTitle, artistName</strong>
              <br />
              <br />
              Example:
              <br />
      ...
      {successLogs.length > 0 && (
        ...
        >
          ...
          {successLogs.map((l, i) => {
            return (
              <Text key={i} color={"green.500"} pl={"1rem"}>
                - {l}
              </Text>
            );
          })}
      ...
      {logs.length > 0 && (
        ...
        >
          ...
          {logs.map((l, i) => {
            return (
              <Text key={i} color={"red.500"} pl={"1rem"}>
                - {l}
              </Text>
            );
          })}
