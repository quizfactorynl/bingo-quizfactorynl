...
import { Button, FormControl, FormErrorMessage, FormLabel, Input, Menu, MenuButton, MenuItem, MenuList, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, UseDisclosureProps, useToast } from "@chakra-ui/react";
import { useRef, useState } from "react";
import { useForm } from "react-hook-form";


const RefCodeModal = (
    {  handler, bingos, codes  }: 
    { 
        handler: UseDisclosureProps, bingos: BingoDocType[], codes: RefCodeDocType[]
    }
) => {
  ...
  >
    ...
    <ModalContent>
      ...
      <form onSubmit={(e)=>{
        ...
        const errors = {
             code: input.code.length != 5 ? {
                 message: 'Code must be 5 characters'
             } : undefined,
             bingo_name: input.bingo_name.length == 0 ? {
        ...
        if(errors.code || errors.bingo_name
            || duplicate
        )
            return;
        
        setLoading(true);
        uploadRefCode(input.code, input.bingo_id, ()=> {
            toast({
                title: 'Success',
      ...
      <ModalBody pb={1} justifyContent={'center'} alignItems={'center'}>
          <FormControl isInvalid={duplicate || (input.errors as any).code != undefined} 
          ...
          >
            ...
            <VerificationCodeInput 
                inputProps={{
                    value: input.code,
                    onChange: (e)=> {
                        setDuplicate(codes.find(code=> code.code == e) != undefined)
                        setInput(prev=> {
                            return {
                                ...prev,
                                code: e +''
            ...
            <FormErrorMessage>
              ...
              {(input.errors as any).code && (input.errors as any).code.message as string}
            </FormErrorMessage>
          </FormControl>

          <FormControl isInvalid={(input.errors as any).bingo_name != undefined} width={'100%'} justifyContent={'center'} mt={'2rem'}>
            ...
            <FormErrorMessage>
              {duplicate && 'bingo already exists\n'}
              {(input.errors as any).bingo_name && (input.errors as any).bingo_name.message as string}
            </FormErrorMessage>
          </FormControl>
