...
import { Button, FormControl, FormErrorMessage, FormLabel, Input, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, UseDisclosureProps, layout, useDisclosure, useToast } from "@chakra-ui/react"
...
const BingoModal = ({ handler, bingos, bingo }: 
  { handler: UseDisclosureProps, bingos: BingoDocType[], bingo?: BingoDocType }
)=> {
    ...
    return <Modal
        ...
        >
          <ModalOverlay />
          <ModalContent>
            <ModalHeader color={'black'}>{bingo ? 'Update' : 'Upload'} Bingo</ModalHeader>
            <ModalCloseButton />
            <form onSubmit={(e)=>{
              e.preventDefault()
              handleSubmit((d)=> {
                ...
                if(!bingo) {
                    ...
                    updateBingo((bingo as any).id , d.title as string, ()=> {
                        toast({
                            title: 'Duplicate Entry',
                            description: 'bingo already exists',
                            status: 'error'
                ...
                <FormControl isInvalid={errors.title !== undefined || duplicate}>
                  ...
                  <Input placeholder="Title" 
                    ...
                    {...register("title", { 
                      ...
                      onChange: (e)=> {
                        setDuplicate(bingos.filter(b=> b.title === e.target.value).length > 0)
                      }
                    })}
                  />
                  <FormErrorMessage>
            ...
            <ModalFooter>
              ...
              <Button onClick={handler.onClose} isLoading={loading}>Cancel</Button>
            </ModalFooter>
          </form>
          </ModalContent>
        </Modal>
